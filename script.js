document.addEventListener('DOMContentLoaded', () => {
    ...
    function canSendFriendRequest() {
        ...
        if (now - lastFriendRequestTime < friendCooldown) {
            alert(`Wait ${Math.ceil((friendCooldown - (now - lastFriendRequestTime)) / 1000)}s before sending another friend request`);
            return false;
        }
    ...
    const guestMessages = ['haha','lol','wow','ok','yea','cool'];
    ...
    function appendChat(msg, user, status, replyToUser = null){
        ...
        replyBtn.onclick = () => { replyTo = user; chatInput.focus(); }; 
        div.appendChild(replyBtn);

        chatWrapper.appendChild(div); 
        chatWrapper.scrollTop = chatWrapper.scrollHeight; 
        chatHistory.push({user, msg, status, replyTo: replyToUser});
    }
    ...
    function sendMessage(){ 
        ...
        if(!currentUser && !guestMessages.includes(msg)) 
            return alert('Guests can only use preset messages'); 
        ...
        const users = JSON.parse(localStorage.getItem('users') || '{}'); 
        const status = currentUser ? users[currentUser].status : null; 
        appendChat(msg, currentUser || 'Guest', status, replyTo); 
        replyTo = null; 
        chatInput.value = ''; 
        lastChatMsg = msg;
    }

    // ===== CONTACT HEADER "CAT" MINIGAME =====
    const contactHeaderLetters = document.querySelectorAll('#contact-header span');
    const catSequence = ['C','A','T']; // the sequence to click
    let catIndex = 0;
        ...
        letter.addEventListener('click', () => {
            if(letter.dataset.letter === catSequence[catIndex]){
                ...
                if(catIndex === catSequence.length){
                    ...
                    window.open('minigame.html', '_blank'); // opens minigame in new tab
                    catIndex = 0; // reset sequence
                }
            } else {
                catIndex = 0; // reset on wrong click
            }
        });
    });

    // Prevent rapid paste spam
    chatInput.addEventListener('paste', (e) => {
        const now = Date.now();
        if(now - lastPasteTime < 500){ // 0.5s between pastes
            e.preventDefault();
            alert('Slow down with pasting!');
        } else {
            lastPasteTime = now;
        }
    });
    ...
    async function resetPasswordHelper() {
        ...
        localStorage.setItem('users', JSON.stringify(users));
        alert(`Password for ${uname} has been reset to "${newPass}". You can now log in with this password.`);
    }

    // Add a "Forgot Password?" button below login
